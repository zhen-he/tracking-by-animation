...
bool BackgroundSubtractorIMBS::apply(InputArray _frame, OutputArray _fgmask, double learningRate)
{
    frame = _frame.getMat();
...
void BackgroundSubtractorIMBS::createBg(unsigned int bg_sample_number) {
	...
	//it is time to compute the fg mask
	...
	}//end else if(bg_sample_number == (numSamples - 1)) 
	    ...
	    }//end for(unsigned int s = 0; s <= bg_sample_number; ++s)
	}//end else bg_sample_number > 0 && bg_sample_number != (numSamples - 1) 
    }//numPixels
